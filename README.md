# Telangana Water Quality SWOT Dashboard

[![GitHub Pages](https://img.shields.io/badge/demo-online-brightgreen.svg)](https://arvind-55555.github.io/SWOT-Dashboard/)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)

Interactive geospatial visualization dashboard for monitoring water quality across 213 stations in Telangana state.

---
### Live Artifact
[![View Artifact](https://img.shields.io/badge/View%20Artifact-%230077B5.svg?style=for-the-badge&logo=claude&logoColor=white)](https://arvind-55555.github.io/Telangana-Water-Quality-Dashboard/index.html)

[![Water Quality Analytics](https://img.shields.io/badge/Water%20Quality%20Dashboard-%23667eea.svg?style=for-the-badge)](https://water-quality-p81j4vv.gamma.site/)

---
## Features

- **Interactive Map** - Leaflet.js-based geospatial visualization
- **213 Monitoring Stations** - Complete coverage across Telangana
- **Color-Coded WQI** - Green (Safe), Orange (Polluted), Red (Highly Polluted)
- **Multiple Base Maps** - Geographic, Terrain, Satellite, Street views
- **Responsive Design** - Works on desktop and mobile devices
- **Support Link** - Contribute via Buy Me a Coffee

## Quick Start

### Online (No Installation)
Visit: https://arvind-55555.github.io/Telangana-Water-Quality-Dashboard/

### Local Development
```bash
git clone https://github.com/Arvind-55555/Telangana-Water-Quality-Dashboard.git
cd Telangana-Water-Quality-Dashboard
./launch.sh
```

## Support This Project

**☕ Buy Me a Coffee:** https://buymeacoffee.com/arvind1111

## Data Coverage

- **Total Stations:** 213
- **Rivers:** Godavari, Krishna, Musi, Manjeera, Kinneresani
- **Data Period:** June 2025
- **Source:** Telangana Pollution Control Board

## Deployment

Deployed on GitHub Pages: https://arvind-55555.github.io/Telangana-Water-Quality-Dashboard/

## Credits

- **Data:** Telangana Pollution Control Board
- **Mapping:** Leaflet.js
- **Inspiration:** Bangladesh SWOT Framework

---

